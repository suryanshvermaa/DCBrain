...
- **Immediate Next Task:** Task 004 — Document Processing Pipeline. See [NEXT_CHAT.md](./NEXT_CHAT.md) and [tasks/004-document-processing/](./tasks/004-document-processing/).
- **Phase:** Phase 1 — Foundation (per [ROADMAP.md](./ROADMAP.md)). Task 001 complete; Task 002 complete; Task 003 complete; Task 004 ready to start.
...
- **Demo Bootstrap RBAC:** In a fresh database, the first registered user is created as `PROJECT_MANAGER` so the hackathon/demo flow can create the first project before full admin user management exists. Existing local demo users were promoted to `PROJECT_MANAGER` on 2026-07-08.
- **Database Migration Status:** Local Docker Postgres has all Task 003 migrations applied, including the split `QUEUED` enum migration and document table/category/version migration.
