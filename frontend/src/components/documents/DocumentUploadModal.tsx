...
export function DocumentUploadModal({ open, progress, uploading, onClose, onUpload }: DocumentUploadModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-gray-950/50 px-4">
      <div className="w-full max-w-2xl bg-[var(--color-surface)] border border-[var(--color-border)] shadow-2xl rounded-lg">
        <div className="flex items-center justify-between border-b border-[var(--color-border)] px-5 py-4">
          <div>
            <h2 className="text-lg font-semibold text-[var(--color-text-primary)]">Upload documents</h2>
            <p className="text-sm text-[var(--color-text-secondary)]">Batch upload up to 50 files.</p>
          </div>
          <button type="button" className="rounded-md p-2 text-[var(--color-text-secondary)] hover:bg-[var(--color-surface-hover)]" onClick={onClose} disabled={uploading} aria-label="Close upload dialog">
            <X className="h-5 w-5" />
          </button>
        </div>
        ...
        <div className="space-y-4 px-5 py-5">
          <label className="block">
            <span className="mb-1 block text-sm font-medium text-[var(--color-text-secondary)]">Category</span>
            <input
        ...
        <div className="flex justify-end gap-3 border-t border-[var(--color-border)] px-5 py-4">
          <button type="button" className="rounded-lg border border-[var(--color-input)] px-4 py-2 text-sm font-medium text-[var(--color-text-secondary)] hover:bg-[var(--color-surface-hover)]" onClick={onClose} disabled={uploading}>
            Cancel
          </button>
          <button
            type="button"
